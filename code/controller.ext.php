...
class module_controller {
    ...
    static function ListRepositories() {
        global $zdbh;
		global $controller;
		$repolist = ctrl_options::GetSystemOption('sentora_root') . "etc/zppy-cache/repo.list";
    ...
    static function ListModules() {
        global $zdbh;
		global $controller;
		$modulelist = ctrl_options::GetSystemOption('sentora_root') . "etc/zppy-cache/package.list";
					...
					if (isset($content[1]) && !fs_director::CheckForEmptyValue($content[1])){
						if (fs_director::CheckForEmptyValue(self::ForbiddenModule($content[0]))) {	
					        ...
					        if ($numrows->fetchColumn() <> 0) {
								$installed = "installed";
								$title = $content[0] . " is already installed on this Sentora server.";
	...
	static function PatchZPPY(){
		global $controller;
		$zppyfolder = ctrl_options::GetSystemOption('sentora_root') . "bin/";
		$zppypatchedfolder = ctrl_options::GetSystemOption('sentora_root') . "modules/repo_browser/bin/";
