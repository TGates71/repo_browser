			...
			if (UrlExist($file)){
				...
				if ($latestversion > $installedversion){
					...
					if ($checkverion['version'] == $latestversion){
						$line = "<div class=\"znotice alert alert-block alert-info\"><h2><img src=\"modules/repo_browser/assets/warning-small.png\"> Version Mismatch!</h2><p>Sentora thinks that " . $modulename . " version is:<b>" . $installedversion . "</b>, but the installed module file is showing that it is actually version:<b>" . $checkverion['version'] . "</b></p><p>This is usually because the module developer failed to update the Sentora database with the new version number when the module was updated.</p><p>If you just upgraded this module and are getting this warning then you can allow Repo Browser to manually update Sentora with the correct version number.</p><p><button class=\"button-loader btn btn-primary fg-button ui-state-default ui-corner-all\" type=\"submit\" name=\"inUpdateVersionDB\" id=\"inUpdateVersionDB\" value=\"" . $checkverion['version'] . ":::" . $modulename . ":::" . $installedversion . "\">FIX VERSION NUMBER</button></p></div>";
					} else {
						$line = "";
					}
